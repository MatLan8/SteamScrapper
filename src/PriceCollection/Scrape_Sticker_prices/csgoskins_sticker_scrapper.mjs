...
const BASE_URL = "https://csgoskins.gg/categories/sticker?order=highest_price";
const OUT_FILE = "../Database/csgoskins_sticker_prices.json";
...
function parseArgs() {
  ...
  if (!rawPage) {
    console.error(
      "Usage: node csgoskins_sticker_scrapper.mjs <startingPage> [--headful]",
    );
    process.exit(1);
  }
...
async function scrapeCurrentPage(page) {
  const items = await page.evaluate(() => {
    ...
    return cards.map((card) => {
      ...
      const collectionEl = card.querySelector(
        '[class*="text-gray-400"][class*="text-sm"][class*="truncate"]',
      );
      ...
      const stickerName = titleEl?.textContent?.trim() ?? null;
      const collection = collectionEl?.textContent?.trim() ?? null;
      const priceText = priceAnchor?.textContent?.trim() ?? null;

      let fullName = null;

      if (stickerName) {
        if (!collection || collection === "Sticker") {
          fullName = `Sticker | ${stickerName}`;
        } else {
          fullName = `Sticker | ${stickerName} | ${collection}`;
        }
      }
  ...
  try {
    ...
    while (true) {
      ...
      if (pageItems.length === 0) {
        console.log("No sticker rows found on page. Stopping.");
        break;
      }
      ...
      for (const item of pageItems) {
        ...
        const existingPrice = results[item.name]?.price;

        if (existingPrice !== convertedPrice) {
          results[item.name] = {
      ...
      console.log(`Sticker rows found on page: ${pageItems.length}`);
      console.log(`Sticker rows changed on page: ${updatedCount}`);
      console.log(
        `Total unique stickers in file: ${Object.keys(results).length}`,
      );
