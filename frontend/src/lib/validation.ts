...
export function validateFloatMultiArgs(
  ...
  const quality = body.quality != null ? String(body.quality).toLowerCase() : "normal";
  ...
  if (workers === null || workers <= 0) errors.workers = "workers must be a positive integer";

  const waitMs = toInt(body.waitMs, 1500);
  if (waitMs === null || waitMs < 0) errors.waitMs = "waitMs must be a non-negative integer";

  if (body.maxSkins != null && body.maxSkins !== "") {
    const m = toInt(body.maxSkins, NaN);
    if (m === null || m <= 0) errors.maxSkins = "maxSkins must be a positive integer";
  }

  if (body.maxListingsPerSkin != null && body.maxListingsPerSkin !== "") {
    const m = toInt(body.maxListingsPerSkin, NaN);
    if (m === null || m <= 0) {
      errors.maxListingsPerSkin = "maxListingsPerSkin must be a positive integer";
    }
  }
...
export function validateSingleUrlArgs(
  body: Record<string, unknown>,
): { valid: boolean; errors: Record<string, string> } {
  const errors: Record<string, string> = {};
  ...
  if (body.currency != null && body.currency !== "") {
    ...
    if (c === null || c <= 0) errors.currency = "currency must be a positive integer";
  }
